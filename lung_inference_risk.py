...
# === 1️⃣ 데이터 전처리 (lung: 전부 숫자화만 하면 됨) ===
def preprocess_lung_dataset(df):
...
def evaluate_synthetic_folder(real_path, synthetic_folder):
    ...
    real_df = preprocess_lung_dataset(real_df)
    ...
    for file in tqdm(os.listdir(synthetic_folder)):
        if file.endswith('.csv'):
            ...
            synthetic_df = preprocess_lung_dataset(synthetic_df)
...
# === 6️⃣ 실행 예시 ===
real_data_path = '/home/khyj/0. phd_DC/DC_original_final/original_lung.csv'
...
results_df.to_csv('lung_inference_risk_A_mean_based.csv', index=False)
print("완료! 결과 저장: lung_inference_risk_A_mean_based.csv")
